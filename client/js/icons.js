...
export function itemIcon(id) {
  ...
  if (name.includes('scimitar')) {
    ...
    // Skill-mastery capes (def.master) get a gold-trimmed outline to match.
    const col = def.tint || '#7a2a3a';
    const tattered = /tattered/.test(name);
    const trimmed = !!def.master;
    g.fillStyle = col;
    ...
    g.closePath(); g.fill();
    g.strokeStyle = trimmed ? '#e8c240' : INK; g.lineWidth = trimmed ? 1.6 : 1; g.stroke();
    g.strokeStyle = shadeHex(col, 0.6); g.lineWidth = 1; g.beginPath(); g.moveTo(16, 8); g.lineTo(16, tattered ? 22 : 25); g.stroke();  // centre fold
